...
## Disk pressure and cache cleanup

When disk pressure blocks verification, look outside the project first for disposable caches or artifacts. Avoid clearing in-project build stores such as `target/`, `.next/`, or local app caches unless they are the immediate blocker and the user accepts the rebuild cost.
