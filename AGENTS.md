...
- Product CLI path: `theorem up` starts/reuses the local node plus `theorem-proxy`; `theorem status` prints node/proxy state plus `/status` counters; `theorem doctor` probes node readiness, GraphQL memory over `graphql_query`, and proxy health; `theorem connect <claude|codex>` writes reversible user-level agent config; `theorem connect <agent> --check` proves routing by incrementing `/status` counters; `theorem disconnect <agent>` restores the saved config from `~/.theorem/connect/`.
- Transitional Codex Desktop opener: `apps/theorem-proxy/scripts/start-proxied-codex-app.sh --restart-codex` starts/reuses the local node and proxy, quits any already-running Codex app, and opens this workspace. Current Codex Desktop 0.142.4 does not honor `codex app -c openai_base_url=...` for model routing, so treat this as an open-workspace helper; durable Desktop routing belongs to `theorem connect codex`, which writes user-level config and verifies `/status.openai_responses_seen`. Double-click `apps/theorem-proxy/scripts/RustyRed-Codex.command` for the same transitional flow.
...
- Resident Qwen/Mistral room heads: `source apps/theorem-proxy/scripts/agent-room-participants-env.sh` after the private `~/.theorem-proxy/qwen.env` and `~/.theorem-proxy/mistral.env` files exist. It sets `THEOREM_AGENT_HEADS=qwen,mistral`, `THEOREM_AGENT_ROOM_RUNNER=1`, `QWEN_MODEL=qwen3.7-max`, and `MISTRAL_MODEL=mistral-small-latest` for the current shell; then start the harness server/local room from that shell.
...
- To point an existing Codex CLI process at the proxy, run with `codex -c 'openai_base_url="http://127.0.0.1:8788/v1"'` or run `theorem connect codex` for persistent user-level routing. Codex Desktop routing requires user-level `~/.codex/config.toml` provider config plus app restart/open; project config and `codex app -c` do not route Desktop model traffic. Current Codex 0.142.4 accepts the local provider with `requires_openai_auth = false` and no `env_key`; adding `env_key` makes Doctor require that env var.
