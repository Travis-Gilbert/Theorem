...
- Transitional Codex Desktop opener: `apps/theorem-proxy/scripts/start-proxied-codex-app.sh --restart-codex` starts/reuses the local node and proxy, quits any already-running Codex app, and opens this workspace. Current Codex Desktop 0.142.3 does not honor `codex app -c openai_base_url=...` for model routing, so treat this as an open-workspace helper until `theorem connect codex` writes user-level config and verifies `/status.openai_responses_seen`. Double-click `apps/theorem-proxy/scripts/RustyRed-Codex.command` for the same transitional flow.
...
- To point an existing Codex CLI process at the proxy, run with `codex -c 'openai_base_url="http://127.0.0.1:8788/v1"'` or add the same `openai_base_url` only to a temporary/profile config. Codex Desktop routing requires user-level `~/.codex/config.toml` provider config plus app restart/open; project config and `codex app -c` do not route Desktop model traffic. Keep the normal global Codex config untouched unless the user explicitly asks for persistent routing.
