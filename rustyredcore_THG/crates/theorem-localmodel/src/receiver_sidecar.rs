...
pub fn spawn_receiver_sidecar(config_path: &Path) -> LocalModelResult<JoinHandle<()>> {
    let config = ReceiverConfig::load(config_path)
        .map_err(|error| LocalModelError::Config(format!("receiver config: {error}")))?;
    ...
    .map_err(|error| LocalModelError::Config(format!("receiver client: {error}")))?;
    let handle = thread::Builder::new()
        .name("theorem-receiver-sidecar".to_string())
        .spawn(move || {
            if let Err(error) = theorem_receiver::run_loop(&config, &client) {
                eprintln!("[theorem-localmodel] receiver sidecar stopped: {error}");
            }
        })?;
    Ok(handle)
}
