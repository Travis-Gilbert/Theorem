...
/// handle. The fetch-cascade engine has no geometry, so it ignores the device
/// point and actuates by `element_id`; semantic activation has no meaning
/// without the Servo fork. Pure mapping (no engine), so it lives in the core and
/// the fetch-cascade `BrowserDriver` impl (rustyred-web) calls it.
pub fn browser_action_for_plan(plan: &ActuationPlan) -> BrowserEngineResult<BrowserAction> {
...
mod tests {
    use super::*;
    use crate::automation::{ActionabilityCheck, RoleOptions};
    ...
    fn snapshot_script_shares_provenance_with_the_selector_bridge() {
        ...
        assert!(crate::automation::SELECTOR_BRIDGE_SCRIPT.contains("roleOf"));
    }
