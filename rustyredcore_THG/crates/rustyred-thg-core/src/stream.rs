...
impl StreamLog {
    ...
    pub fn latest_token(&self) -> u64 {
        self.by_token.get_max().map(|(token, _)| *token).unwrap_or(0)
    }
...
impl StreamRegistry {
    ...
    pub fn read(&mut self, actor: &str, stream_key: &str, advance: bool, limit: usize) -> StreamDelta {
        ...
        let events = self
            ...
            .unwrap_or_default();
        let new_cursor = events.last().map(|event| event.ordering_token).unwrap_or(cursor);
    ...
    pub fn read_subscribed(&mut self, actor: &str, advance: bool, limit: usize) -> Vec<StreamDelta> {
    ...
    pub fn latest_token(&self, stream_key: &str) -> u64 {
        self.streams.get(stream_key).map(StreamLog::latest_token).unwrap_or(0)
    }
}
...
mod tests {
    ...
    fn subscription_set_controls_which_deltas_a_read_returns() {
        ...
        let keys = both.iter().map(|d| d.stream_key.clone()).collect::<BTreeSet<_>>();
