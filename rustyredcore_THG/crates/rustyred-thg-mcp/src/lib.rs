...
fn instant_kg_payload(
    ...
    let payload = match operation {
        ...
        "ppr" => {
            ...
            let alpha = arguments.get("alpha").and_then(Value::as_f64).unwrap_or(0.15);
            let epsilon = arguments
                .get("epsilon")
                .and_then(Value::as_f64)
                .unwrap_or(1e-4);
...
mod tests {
    ...
    fn graphql_epistemic_domain_matches_flat_tools() {
        ...
        assert_eq!(applied["shadows_written"].as_u64(), Some(2), "enrich should write shadows: {applied}");
    ...
    fn graphql_introspect_exposes_epistemic_domain() {
        let (provider, config) = fixture();
        let sdl = call_tool_json(&provider, &config, "graphql_introspect", json!({ "tenant": "smoke" }));
        let sdl = sdl.as_str().expect("introspect should return an SDL string");
        for fragment in [
            "epistemicNeighbors",
            "epistemicFrontier",
            "compileSubgraph",
            ...
            assert!(sdl.contains(fragment), "SDL missing epistemic-domain {fragment}:\n{sdl}");
        }
        assert!(
            !sdl.to_lowercase().contains("tenant"),
            "no GraphQL field may carry a tenant argument:\n{sdl}"
    ...
    fn graphql_code_search_matches_flat_tool() {
        ...
        assert_eq!(gql["data"]["codeSearch"]["operation"], json!("search"), "{gql}");
    }
    ...
    fn graphql_introspect_exposes_code_domain() {
        ...
        for fragment in [
            ...
            assert!(sdl.contains(fragment), "SDL missing code-domain {fragment}:\n{sdl}");
        }
        assert!(
            !sdl.to_lowercase().contains("tenant"),
            "no GraphQL field may carry a tenant argument:\n{sdl}"
    ...
    fn graphql_harness_kg_matches_flat_tools() {
        ...
        assert_eq!(
            gql["data"]["harnessKgImpact"]["seed"], json!("node:a"),
            "Ada should resolve to node:a: {gql}"
        );

        // harnessKgStatus also routes through the same payload and answers without error.
        let status = graphql_tool_call(&provider, &config, "graphql_query", "query{ harnessKgStatus }", Value::Null);
    ...
    fn graphql_introspect_exposes_harness_kg_domain() {
        ...
        for fragment in [
            ...
            assert!(sdl.contains(fragment), "SDL missing harness-kg {fragment}:\n{sdl}");
        }
        assert!(
            !sdl.to_lowercase().contains("tenant"),
            "no GraphQL field may carry a tenant argument:\n{sdl}"
    ...
    fn graphql_clusters_match_flat_tools() {
        ...
        let gql_skills = graphql_tool_call(&provider, &config, "graphql_query", "query{ skillList }", Value::Null);
        assert_no_graphql_errors(&gql_skills);
        let flat_skills = call_tool_json(&provider, &config, "skill_list", json!({ "tenant": "smoke" }));
        assert_eq!(gql_skills["data"]["skillList"], flat_skills, "skillList must match flat skill_list");

        let gql_run = graphql_tool_call(&provider, &config, "graphql_query", "query{ harnessRun(runId:\"missing\") }", Value::Null);
        assert_no_graphql_errors(&gql_run);
        let flat_run = call_tool_json(&provider, &config, "harness_run", json!({ "tenant": "smoke", "run_id": "missing" }));
        assert_eq!(gql_run["data"]["harnessRun"], flat_run, "harnessRun must match flat harness_run");
        ...
        let gql_jobs = graphql_tool_call(&provider, &config, "graphql_query", "query{ jobList }", Value::Null);
        assert!(
            gql_jobs["data"].is_null() && !gql_jobs["errors"].as_array().map(|e| e.is_empty()).unwrap_or(true),
            "jobList must route to the backend (not silently return empty): {gql_jobs}"
        );
        let gql_jobs_err = gql_jobs["errors"][0]["message"].as_str().unwrap_or_default();
        assert!(
    ...
    fn graphql_ensemble_register_select_round_trips() {
        ...
        let pack_hash = registered["data"]["ensembleRegister"]["pack"]["pack_content_hash"]
            ...
            .to_string();
        assert!(!pack_hash.is_empty(), "ensembleRegister must return a pack hash: {registered}");
    ...
    fn graphql_introspect_exposes_cluster_domains() {
        ...
        for fragment in [
            ...
            assert!(sdl.contains(fragment), "SDL missing cluster {fragment}:\n{sdl}");
        }
        assert!(
            !sdl.to_lowercase().contains("tenant"),
            "no GraphQL field may carry a tenant argument:\n{sdl}"
    ...
    fn graphql_default_surface_hides_covered_flat_tools() {
        ...
        assert!(default_names.iter().any(|n| n == "recall"), "default mode keeps flat recall");
        assert!(
            default_names.iter().any(|n| n == "rustyred_thg_graph_neighbors"),
            "default keeps flat neighbors"
        );
        assert!(default_names.iter().any(|n| n == "graphql_query"), "graphql_query always listed");
        ...
        for hidden in [
            ...
            assert!(!cut_names.iter().any(|n| n == hidden), "cutover must hide covered flat tool {hidden}");
        }
        for kept in ["graphql_query", "graphql_introspect", "graphql_mutate"] {
            assert!(cut_names.iter().any(|n| n == kept), "cutover must keep {kept}");
        }
    ...
    fn graphql_default_surface_completes_task_through_graphql_alone() {
        ...
        assert!(
            !tool_names(&provider, &config).iter().any(|n| n == "remember"),
        ...
        assert!(
            remembered["data"]["rememberMemory"]["id"].as_str().is_some(),
        ...
        assert!(
            recalled["data"]["memory"].as_array().map(|a| !a.is_empty()).unwrap_or(false),
