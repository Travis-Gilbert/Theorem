...
impl EpistemicQuery {
    ...
    async fn epistemic_neighbors(
        ...
        with_invoker(|inv| Ok(Json(inv.epistemic_neighbors(args.clone()).map_err(map_err)?)))
    }
    ...
    async fn epistemic_frontier(
        ...
        with_invoker(|inv| {
            Ok(Json(
                inv.epistemic_dirty_frontier(args.clone()).map_err(map_err)?,
            ))
        })
    }
    ...
    async fn compile_subgraph(&self, content_ids: Vec<String>) -> GqlResult<Json> {
        ...
        with_invoker(|inv| {
            Ok(Json(
                inv.epistemic_compile_subgraph(args.clone()).map_err(map_err)?,
            ))
        })
    }
    ...
    async fn shadow_ppr(
        ...
        with_invoker(|inv| Ok(Json(inv.epistemic_shadow_ppr(args.clone()).map_err(map_err)?)))
    }
}
