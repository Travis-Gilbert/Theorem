...
impl ClustersQuery {
    ...
    async fn ensemble_select(&self, input: Json) -> GqlResult<Json> {
        with_invoker(|inv| Ok(Json(inv.ensemble("select", input.0.clone()).map_err(map_err)?)))
    }
...
impl ClustersMutation {
    ...
    async fn skill_publish(&self, input: Json) -> GqlResult<Json> {
        with_invoker(|inv| Ok(Json(inv.skill("publish", input.0.clone()).map_err(map_err)?)))
    }
    ...
    async fn ensemble_register(&self, input: Json) -> GqlResult<Json> {
        with_invoker(|inv| Ok(Json(inv.ensemble("register", input.0.clone()).map_err(map_err)?)))
    }
