...
#[test]
fn text_region_rehydrates_after_peer_restart() {
    let tmp = tempfile::tempdir().unwrap();
    let log = Arc::new(Mutex::new(WorkingLog::new()));
    let data_dir = tmp.path().join("codex");
    let region = NoteAdapter::new("note:restart").body_region("intro");

    {
        let mut codex = peer("codex", "note:restart", 11, log.clone(), &data_dir);
        codex
            .text_region(&region)
            .unwrap()
            .push("restart durable text")
            .unwrap();
        assert_eq!(
            codex.text_region_contents(&region).as_deref(),
            Some("restart durable text")
        );
    }

    let mut reopened = peer("codex", "note:restart", 11, log, &data_dir);
    reopened.text_region(&region).unwrap();

    assert_eq!(
        reopened.text_region_contents(&region).as_deref(),
        Some("restart durable text"),
        "the Yrs update and the DocTree namespace must survive a peer restart"
    );
}
