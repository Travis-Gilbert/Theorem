...
| `commonplace` | `commonplace-desktop`: static-export Next.js CommonPlace desktop frontend for the Tauri shell. Reads/writes the local node over `/graphql`, subscribes to `/v1/items/stream`, and wraps the existing native command bridge for keychain, receiver, coordination, sync, and co-browser panels. |
