...
| 5 intra-agent scratchpad loop (propose/critique/synthesize/publish, router) | done for deterministic binding loop, including North Star stages 1-6; live adapters remain | codex | - |
...
Remaining real-build work = Lane B. Step 3 now has a pure `theorem-harness-core::AgentHeadRegistry` slice that resolves active heads to fake transport endpoints, emits a `HEADS.PROBED` transition payload, preserves `credential_ref` without credential values, and rejects inactive/unknown heads before invocation. Step 5 now has a deterministic `run_fake_intra_agent_loop` scaffold plus a fake-first `HeadInvoker` seam. North Star binding stages 1-6 extend that scaffold with persisted scratchpad DAG/epistemic relation edges, per-capability reliability routing, binding-level synthesis verification receipts, marginal-value budget decisions, bounded convergence rounds, and `OUTCOME.RECORDED` writeback into per-head/domain capability reliability. The runtime `ProviderHeadInvoker` now resolves credentials at call time, calls API-backed heads, parses provider claims into receipts, and backs production `composed_agent_run`. Step 8 now has the remaining Theseus app surface registered as graph-visible metadata through `register_theseus_app_affordances`: explicit `theorem_grpc.*` wrappers with transport, timeout, failure receipt shape, permissions, writeback policy, and cost metadata. The live `theorem_grpc` invocation adapter and charter-surface reintegration remain open. All build on the committed Lane A types.
