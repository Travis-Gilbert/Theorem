...
Green in `rustyred-thg-code`, `rustyred-embedded`, and `rustyred-workspace`:

- `IngestCodebaseInput::materialize_symbol_name_index` and the matching
  `IngestConfig` flag opt into W1's persistent inverse-index primitive without
  changing default full-ingest cost.
- When the flag is enabled, ingest materializes `CodeSymbolName` bucket nodes
  keyed by repo/generation/name. Each bucket stores deterministic target refs
  (`symbol_id`, `file_path`, `line`) and `target_count`; the plugin manifest now
  advertises the label.
- Durable ingest-job replay preserves the flag, so queued W1-enabled ingests do
  not silently drop the bucket index on restart.
- `index_source_file_write_in_store` is the W1.1 source-file write primitive:
  it parses exactly the written source file from caller-provided bytes, commits
  the `CodeFile`/`CodeSymbol` nodes without walking the repo, and carries
  unchanged current graph nodes forward to preserve the latest-generation view.
- The first W1.3 edge-rewrite slice is green for changed-file outgoing edges:
  the source-file write path materializes only touched `CodeSymbolName` buckets,
  tombstones stale outgoing `CALLS`/`DEPENDS` edges from changed symbols, and
  upserts the new outgoing edges from touched-name lookups.
- The reverse W1.3 slice is green for newly defined names: carried symbols that
  mention a changed definition name are reconstructed from persisted
  `CodeFileText`, stale matching outgoing edges are tombstoned, and incoming
  `CALLS`/`DEPENDS` edges to the new target are upserted without a repo walk.
- The W1.4 embedding proof is green: when the source-file write path commits
  `CodeSymbol` upserts into a hooked store, `incremental_embed_hook` refreshes
  the 64-dim `embedding`; token-equivalent whitespace edits stay idempotent and
  token-changing edits move the vector.
- The generic embedded post-`File`-write hook is green: `rustyred-embedded`
  exposes an opt-in `FileWriteHook` seam after `fs_write`/`fs_write_batch`
  commit File nodes and persist the DocTree; `rustyred-workspace` installs a
  CodeCrawler hook that feeds direct source-file writes into
  `index_source_file_write_in_store` without going through W3 sync-back.
- `apps/rustyred-workspace` wires the W3 sync-back seam to W1 through
  `sync_back_sources_indexed`: only changed source files are re-written to the
  DocTree and indexable changed files call `index_source_file_write_in_store`
  with the already-read source bytes.

Validation: `ingest_materializes_symbol_name_bucket_index` proves a fixture
ingest writes a `helper_len` bucket that points at the current `CodeSymbol`.
`plugin_manifest_describes_code_graph_capability` proves the label is advertised.
`source_file_write_indexes_one_file_without_repo_walk` proves the W1.1
single-file path. `source_file_write_rewrites_touched_outgoing_edges` proves an
add-call and remove-call edit produce the same visible edge set as a clean full
ingest while preserving an unrelated edge and keeping bucket lookups bounded by
names in the changed file. `source_file_write_touches_only_changed_name_buckets_in_large_repo`
proves the same bound in a 250-symbol fixture. `source_file_write_recomputes_incoming_edges_for_new_definition`
proves a carried caller links to a newly defined target and matches a clean full
ingest. `source_file_write_refreshes_embedding_via_hook` proves W1.4 embedding
refresh/idempotency through the hooked on-write path. Changed-package clippy is
clean. `embedded_file_write_hook_indexes_code_without_sync_back` proves direct
embedded File writes maintain CodeCrawler edges without W3 sync-back. The existing
`indexed_sync_back_updates_code_graph_for_changed_source` proves a materialized
workspace edit syncs back through W3 and refreshes CodeCrawler edges.
`on_write_path_collapses_explicit_code_maintenance_round_trips` is the W1.5
collapse measurement: the explicit maintenance path takes 6 counted agent round
trips and 20 proxy tokens, while the on-write path returns the same "helper is
called by caller" query result in 3 counted round trips and 14 proxy tokens.

Still open for W1: none. W4 remains the separate real-encoder quality swap.
