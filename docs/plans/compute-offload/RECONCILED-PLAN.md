...
| Axis / piece | Theseus-era note | Theorem now (built this session, PR #61) | Gap to close |
| --- | --- | --- | --- |
| Move 1 symbolic offload | planned, primary test | `rustyredcore_THG/crates/rustyred-thg-offload` now carries the implemented planner/routing stack: `OperationPlanner` emits an `OffloadPlan`, `GraphAffordanceEngine` provides CPU graph receipt helpers for PageRank/community/shortest-path, and `plan_from_json` is the MCP gateway entrypoint for planner requests. | Wire the remaining executable substrate affordances behind the returned plan: Datalog (`datafrog`), probabilistic (Beta-Bernoulli), Z3/constraint, and more graph algos. Route real query ops to those execution surfaces after the planner selects them. |
| Move 2 pushdown | planned (DBPlanBench) | `OperationPlanner` reorders true `PredicateFilter` operations before model-heavy operations when dependencies allow. | Broaden from operation-list planning to real query decomposition and benchmark against DBPlanBench-style workloads. |
| Move 3 fusion | planned | `OperationPlanner` discounts adjacent model operations sharing a `fusion_key` and records an operator-fusion rewrite. | Batch real LLM calls; first target = RustyWeb crawl path. |
| Move 4 cascade | planned (45-85%) | `IsotonicCalibrator` + `CascadeRouter` implement the calibrated cheap-model to expensive-model routing primitive. | Feed it token-level uncertainty and benchmark-ledger calibration data from real model traffic. |
| Move 5 reuse | planned (the moat) | `InMemoryComputationCache` is a version-keyed cache arm in the planner, and stale graph-version entries are rejected before selection. | Back the cache with graph-provenance storage keyed on op+inputs plus a staleness/invalidation policy keyed on graph version. |
| **The query planner** | "the architectural piece that doesn't exist" | `OperationPlanner` decomposes the provided operation set, considers cache/CPU/model candidates, applies pushdown/fusion rewrites, and returns a replayable `OffloadPlan` with cost and ledger entries. | Connect the planner to first-class query decomposition and train the learned cost router. The Pairformer's richer role = *operation-level cost routing*, not just participant relevance. |
| Cost test / benchmark ledger | Gate 0 / Gate 1 (A/B0/B1/B2) + cascade arm + reuse arm; pre-registered | `LedgerEntry` and `OffloadPlan::ledger_entries()` record token and GPU-second savings per planner decision. | Build the full benchmark ledger; **it doubles as the router's training data + the cascade's calibration data** (one run, three payoffs). |

Net: **the planner/routing stack exists natively, with CPU graph helpers and ledger receipts available in-crate.** The remaining build is to wire executable substrate affordances, real query decomposition, graph-backed reuse, and the benchmark ledger.
