...
- The standalone binary and commands from `SPEC-LOCAL-PROXY-MVP.md`: `brew install theorem-proxy`, `theorem-proxy proxy`, `theorem-proxy wrap -- claude`, prebuilt per-platform binaries, the `curl ... | sh` installer, CPU-only first-run default.
...
Build: on the Commonplace sign-in and landing surface, a download control that detects the visitor's OS and offers the matching path: a copy-paste `brew install theorem-proxy` line with a copy button for macOS and Linux-with-brew, a `curl ... | sh` line for Linux without brew, and a direct binary download as the fallback. Below it, a copy-paste run line (`theorem-proxy proxy`) and the connect line (`theorem-proxy wrap -- claude`), each with a copy button. The block shows the three steps in order: install, run, connect.
...
### 5. Doctor command
Build: `theorem-proxy doctor` checks the chain and reports each link: the proxy is listening, the substrate identity is present, an upstream model credential is reachable, and Claude Code's `ANTHROPIC_BASE_URL` points at the local proxy. Each check reports pass or the exact fix.
Acceptance: `theorem-proxy doctor` on a correctly connected machine reports all links green, and on a machine where Claude Code is not pointed at the proxy it reports that link red with the fix. Verify both states.
...
Acceptance: clicking Connect in Commonplace produces the same connected state that `theorem-proxy wrap -- claude` produces. Verify by connecting through the button and confirming a Claude Code session runs through the proxy.
...
| 5 | No chain check | `theorem-proxy doctor` reports each link with the exact fix | CLI entry | Build | Doctor is green when connected, names the red link otherwise | [-] |
...
Confirm the exact CLI command strings against the repo entry (`theorem-proxy proxy` / `theorem-proxy wrap -- claude` / `theorem login` / `theorem-proxy doctor`), the harness identity issuance flow the site already uses, the prebuilt-binary release artifact names the Homebrew formula will reference, and the Claude Code settings path the Connect control and `theorem-proxy wrap -- claude` write. Build against the real strings.

## Acceptance for the whole flow

A stranger on the Commonplace site copies one install line, runs one command, runs the connect command, and has Claude Code working through the proxy, with `theorem-proxy doctor` green, and the only identity step was signing in. Verify the whole chain on a clean machine, offline for the model-independent steps.
...
- `theorem login`, `theorem-proxy doctor`, first-run guidance: the CLI entry binary.
