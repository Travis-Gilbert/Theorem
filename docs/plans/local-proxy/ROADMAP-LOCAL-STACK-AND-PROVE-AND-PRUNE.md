...
## Phase A -- local Theorem node on the SSD (Valkey warm cache + proxy)

Goal: a local running Theorem node whose data + cache live on the SSD, with the proxy pointed at its live memory, so memory + coordination are local (bypassing the degraded Railway remote) and the harness runs in-process. This is the substrate that makes Prove-and-Prune real.
...
- **A.1 Valkey on the SSD.** Install `valkey-server` (brew); a `valkey.conf` with `dir /Volumes/SSD Samsung/theorem-valkey`, `appendonly yes`, a fixed port; a launch script. Acceptance: `valkey-server` runs, persists AOF to the SSD, `PING` -> `PONG`, survives restart.
- **A.2 Local substrate on the SSD.** Stand up the chosen node surface with its `RedCoreGraphStore`/object-store `data_dir` on the SSD, and wire Valkey as the warm/cold cache tier (via `RedisGraphStore` or the cold-object path). Acceptance: the node starts, writes land on the SSD, survive a process restart (recover from AOF/snapshot).
...
- **B.5 (spec D5) Built-in measurement + `theorem doctor` readout.** The proxy A/Bs injection on/off; tracks rediscovery rate, collisions prevented, tokens saved; `doctor` surfaces it. This is the answer to "does the harness help" with numbers. Acceptance per spec D5.

## Phase C -- D5 install ergonomics + remaining proxy MVP

- **C.1 SPEC-LOCAL-PROXY-MVP D5:** brew tap + `curl ... | sh` + `theorem wrap claude` (one-command connect) + CPU-only default. Turns the local stack into one command instead of the manual `ANTHROPIC_BASE_URL` export.
...
- **C.3 SPEC-ONECLICK-ONBOARDING:** site download + three-step copy-paste, `theorem login` (account -> substrate key), `theorem doctor` chain-check (+ the value readout from B.5), Connect-button parity.
