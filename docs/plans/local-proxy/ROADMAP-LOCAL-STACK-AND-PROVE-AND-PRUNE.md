...
- Specs: `docs/plans/local-proxy/SPEC-PROXY-PROVE-AND-PRUNE.md` (the 5 deliverables) and `docs/plans/local-proxy/SPEC-GRAPHQL-CONTRACT-AND-CLI.md` (current GraphQL-contract-first capability + CLI connect lane). `SPEC-REST-FIRST-CAPABILITIES-AND-CLI.md` is a tombstone only. Source specs `SPEC-LOCAL-PROXY-MVP.md`, `SPEC-PROXY-RESIDENT-CAPABILITIES.md`, `SPEC-ONECLICK-ONBOARDING.md` still in `~/Downloads` -- MOVE them into `docs/plans/local-proxy/`.
