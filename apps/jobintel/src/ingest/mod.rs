...
pub fn load_slugs(path: &str) -> Result<Vec<CompanySlug>> {
    ...
    let raw = std::fs::read_to_string(&resolved).map_err(|e| {
        JobIntelError::Ingest(format!(
            "could not read slugs file '{}': {e}",
            resolved.display()
        ))
    })?;
...
mod tests {
    ...
    #[test]
    fn missing_relative_slugs_path_falls_back_to_crate_dir() {
        let path = resolve_input_path("__missing__/slugs.toml");

        assert_eq!(
            path,
            Path::new(env!("CARGO_MANIFEST_DIR")).join("__missing__/slugs.toml")
        );
    }
}
