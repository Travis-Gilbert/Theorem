...
- Last sync recorded in `README.md` (2026-06-19; added `crates/commonplace` (CommonPlace consumer data layer: F1 Item/Collection/Tag object model + F2 auto-structuring ingest) and `apps/commonplace-api` (F3 interoperability API seam: typed consumer GraphQL profile over the object model + per-instance API-key auth, Axum/async-graphql), plus `apps/rustyred-embedded`, North Star E0 embedded mode: link the crate and run RustyRed in-process over a local directory via `Engine` over the `SharedStore` seam, no server; and `crates/theorem-copresence`, the co-presence substrate peer + surface-adapter seam: graph-CRDT structure via stamped mutations + the `ThgExecutor` command path, yrs 0.27 sequence-CRDT text regions, and working-log presence, with one in-substrate note adapter and a two-peer convergence proof; Codex-built, CC-verified). Bump it when you re-sync.
