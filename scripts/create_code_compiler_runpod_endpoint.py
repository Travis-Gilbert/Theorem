#!/usr/bin/env python3
"""Create a RunPod Serverless endpoint for Theorem code compiler bursts."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Create a RunPod template and endpoint for code compiler bursts.",
    )
    parser.add_argument("--env-file", help="Optional .env file with RUNPOD_API_KEY.")
    parser.add_argument("--rest-base", default=REST_BASE)
    parser.add_argument(
        "--image",
        default="ghcr.io/travis-gilbert/theorem-code-compiler-worker:latest",
    )
    parser.add_argument(
        "--template-id",
        help="Use an existing RunPod template id instead of creating a template.",
    )
    parser.add_argument("--template-name", default="theorem-code-compiler-worker")
    parser.add_argument("--endpoint-name", default="theorem-code-compiler-burst")
    ...
    parser.add_argument("--execution-timeout-ms", type=int, default=1_800_000)
    ...
    parser.add_argument(
        ...
        help="SentenceTransformers model loaded by the worker.",
    )
    ...
    template_body = {
        ...
        "dockerStartCmd": [],
        "env": {
            "THEOREM_CODE_COMPILER_MODEL_ID": args.model_id,
        },
        "isPublic": False,
        "isServerless": True,
        "ports": [],
        "readme": "Theorem Rust-native code compiler burst worker.",
    }
    ...
    endpoint_body = {
        ...
        "scalerValue": 5,
        "computeType": "GPU",
        "gpuTypeIds": args.gpu_type_id or ["NVIDIA L40S", "NVIDIA RTX A5000"],
    }
