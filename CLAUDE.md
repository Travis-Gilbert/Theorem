...
| `theorem-receiver` | Dispatch-queue receiver (replaces the rejected GitHub-Actions dispatcher): an outbound-only claim loop against the cloud harness that spawns the locally-installed `claude` / `codex` CLI in a mapped worktree. Lane detection via PATH scan; `claude -p` / `codex exec`; `ANTHROPIC_API_KEY` stripped from the child env; branch `job/{job_id}`; idempotent Failed-on-exit fallback. No engine state, no inbound port, no stored tokens. Heads are shaped as adapters behind one `HeadAdapter` trait (`head.rs`, the FDW-for-models pattern): a future head lands as config plus one impl (head_id/detect/spawn_cmd/intent_template/env_policy/receipt_parser), not a rewrite; trust tiers + selection priors live in the Ensemble registry, not a parallel one. Builds as a lib (Option A embed) and a bin (Option B standalone). Plan: `docs/plans/dispatch-queue/`. |
...
- **Dispatch queue**: implemented per `docs/plans/dispatch-queue/HANDOFF.md` (supersedes the GitHub-Actions draft). A typed `Job` node lives in the GraphStore beside `RunState`: pure domain in `theorem-harness-core/job.rs` (Job + JobKind/Priority/TargetHead/JobStatus, idempotency-key hash, lane matching, lifecycle guard), persistence + the six push verbs in `theorem-harness-runtime/job_queue.rs` (submit/status/cancel/promote/claim/complete over any `GraphStore`, JobEvent lifecycle log, `JOB_FOR_SPEC`/`DISPATCHED_AS`/`PRODUCED` edges). The verbs are exposed as bearer+tenant MCP tools in `rustyred-thg-mcp` (writes read-only-gated) and wired on the product server's `ProductMcpBackend` (`rustyred-thg-server`) through the `RuntimeTenantMirror`. Execution is `theorem-receiver`: a local spawn of the already-authenticated `claude`/`codex` CLI, zero GitHub Actions/runners/PATs/OAuth tokens. 39 new tests green (core 6, runtime 12, mcp 2, receiver 19). `spawn_session` + `.github/workflows/theorem-handoff.yml` stay as an untouched legacy lane. Deferred (named): SSE jobs-channel wake (polling meanwhile), parallel `capacity > 1` dispatch, the live end-to-end run (the receiver is bootstrap-built, dispatch needs it running).
