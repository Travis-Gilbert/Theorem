...
| `theorem-receiver` | Dispatch-queue receiver (replaces the rejected GitHub-Actions dispatcher): an outbound-only claim loop against the cloud harness that spawns the locally-installed `claude` / `codex` CLI in a mapped worktree. Lane detection via PATH scan; `claude -p` / `codex exec`; `ANTHROPIC_API_KEY` stripped from the child env; branch `job/{job_id}`; idempotent Failed-on-exit fallback. No engine state, no inbound port, no stored tokens. Builds as a lib (Option A embed) and a bin (Option B standalone). Plan: `docs/plans/dispatch-queue/`. |
