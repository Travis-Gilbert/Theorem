...
| `apps/theorem-proxy/` | `theorem-proxy`: local Anthropic Messages passthrough proxy on `ANTHROPIC_BASE_URL` that makes harness memory ambient (no MCP round-trip). Standalone (Axum/reqwest/ureq). `proxy` forwards `POST /v1/messages` byte-for-byte (streaming/SSE) and injects relevance-ranked memory at the cache-stable suffix (last user message only). Memory is a `MemorySource` trait: `DirectoryMemorySource` (fast default), `HttpMemorySource` (node `graphql_query { memory }`, fail-open). `/status` exposes routing counters for Anthropic Messages and OpenAI Responses. `wrap -- <cmd>` = one-command connect; `doctor` = GraphQL memory + proxy stack check; `--membrane-threshold` samples oversized `tool_result`. Scripts: `start-proxied-session.sh`, `start-proxied-codex-session.sh`, transitional Desktop opener `start-proxied-codex-app.sh` + `RustyRed-Codex.command` (opens workspace only until `theorem connect codex` owns user-level Desktop config), `agent-room-participants-env.sh` (loads private Qwen/Mistral env files for resident room heads), `node-local.sh` (embedded RedCore node on SSD; auto-detects a local OpenAI-compat embedding endpoint, else hash), `seed-node.py`. Wrapper CLI: `theorem up`, `theorem connect <claude|codex>`, `theorem connect <agent> --check`, `theorem disconnect <agent>`. Embedder is endpoint-agnostic (`QWEN3_EMBEDDING_4B_URL`: Ollama/mistral.rs/any). Plans: `docs/plans/local-proxy/`. |
...
| `apps/theorem-federation/` | `theorem-federation`: standalone default-off direct peer federation lane. Persists an Iroh endpoint key in the node data dir, maps sha256 substrate identity to BLAKE3 transfer links, frames SubstratePeer deltas/text/presence over the federation ALPN, derives iroh-gossip awareness topics, gates inbound contributions through per-EndpointId EMA trust, and exposes `theorem federation status|identity|doctor|serve`. Spec: `/Users/travisgilbert/Downloads/SPEC-IROH-BLAKE3-FEDERATION-TRANSPORT.md`. |
